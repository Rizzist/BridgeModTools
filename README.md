...
Version 2.4 keeps saved deleted-row author identity actionable. Click the restored avatar or author name to open Discord's native compact profile popout, or use the row toolbar to copy the account username or apply a fixed seven-day server timeout. The popout is anchored to the clicked restored header and closes on scroll, resize, route change, outside click, or Discord's own close action; it never falls back to the full profile modal. Timeout is shown only in server channels and always requires a confirmation naming the archived author; Discord still enforces the moderator's current permission and role hierarchy.
...
Version 2.7.1 fixes deletion persistence across channel switches and startup races. Persistence is acknowledged separately from native retention. Exact rendered rows are captured on deletion even in a hidden tab, pending captures keep their original channel across navigation, and captured deletion payloads retry with bounded backoff until the archive confirms them. A retained native row that has not yet been saved shows **DELETED · SAVING**; the popup reports pending capture/storage instead of claiming success. The MAIN-world hook keeps a bounded ID-only retry ledger and replays unacknowledged deletions on channel changes and recovery. Previously captured messages can be confirmed while viewing another channel; messages never rendered still cannot be reconstructed from IDs alone. Clear, pause, and per-record removal invalidate pending work, release unsaved retained native copies, and prevent old retries from resurrecting records. If 5,000 unresolved IDs fill the retry ledger, new deletions follow Discord's native removal path and capture reports degraded support. If the separate 5,000-ID discarded-row safety bound is exhausted, capture stops safely until Discord is reloaded rather than forgetting an exclusion and saving a discarded row again.

Version 2.7.2 adds transient timed-out status to restored authors and makes deleted-message avatar/name clicks open Discord's native compact profile popout beside the clicked author. The popout uses the current channel, guild, and cached Discord user, cleans itself up on viewport or route changes, and fails closed if Discord's native component is unavailable; it never falls back to the full profile modal.

Version 2.7.3 keeps user mentions inside a restored deleted body as Discord-style interactive mentions. At deletion time, BridgeModTools binds only MessageStore-verified user mention IDs to the exact already-rendered `@label` ranges; clicking one opens the same native compact profile popout. Role mentions, `@here`, `@everyone`, malformed ranges, and ambiguous Discord DOM variants remain inert instead of being guessed.

Restored deleted rows show a compact clock when Discord's current guild-member cache says that author is timed out. The status is checked in bounded batches, refreshed after member updates and timeout actions, and removed at its exact expiry; native live rows keep Discord's own indicator. Timeout state is transient and guild-scoped—it is never written into the message archive or exported, and an unavailable member cache hides the icon instead of guessing. Previously captured native timeout SVGs are removed during archive normalization so an expired timeout cannot remain as a stale author badge.
...
- A `document_start`, main-world adapter discovers Discord's `MessageStore`, signals genuine edits, and retains cached records during single or bulk deletion. It sends validated channel/message IDs plus an edit timestamp/ordering token; a retained deletion may additionally carry up to 50 MessageStore-verified mentioned-user IDs with non-identifying role/broadcast placeholders so the isolated script can bind them to the rendered DOM. Message text, authentication tokens, cookies, role IDs, and store objects never cross that bridge. Up to 5,000 unacknowledged deletion IDs are held in page memory for replay; overflow is reported as degraded capture, not a successful save.
- Live/deleted-row profile and timeout actions pass only validated message/user/guild IDs and a bounded clicked-element rectangle through the document-bound service worker. The worker derives the channel and guild from the current Discord route, rechecks that route in the MAIN world, proves timeout authorship against Discord's current MessageStore or the exact saved deleted record, allows only compact-profile-open or fixed seven-day timeout, and invokes Discord's structurally discovered native client components/actions. It does not read a token or construct a Discord REST request; missing or changed native modules fail closed.
...
- Current member timeout status is read transiently from Discord's local `GuildMemberStore` only for mounted restored rows. It is not stored, exported, fetched from Discord's API, or inferred when the member is absent from the local cache.
